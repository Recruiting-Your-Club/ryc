...
class ClubAnnouncementFacadeTest {
  ...
  void givenClubsAndAnnouncements_whenGetAllClubs_thenReturnsClubsWithStatuses() throws Exception {
    // Given
    ...
    when(announcementService.getStatusByClubId(clubId1)).thenReturn(AnnouncementStatus.CLOSED);
    when(announcementService.getStatusByClubId(clubId2)).thenReturn(AnnouncementStatus.RECRUITING);
