...
public record ClubUpdateResponse(
    ...
    @Schema(description = "동아리 카테고리") @NotNull(message = "club category shouldn't be null")
        Category category,
    @Schema(description = "동아리 태그 리스트") @NotNull(message = "club tag list shouldn't be null")
        List<ClubTag> clubTags,
    @Schema(description = "동아리 요약 리스트") @NotNull(message = "club summary list shouldn't be null")
        List<ClubSummary> clubSummaries,
    @Schema(description = "동아리 상세 이미지 리스트")
        @NotNull(message = "club detail image list shouldn't be null")
