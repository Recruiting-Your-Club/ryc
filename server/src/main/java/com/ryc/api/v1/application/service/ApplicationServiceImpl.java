...
public class ApplicationServiceImpl implements ApplicationService {
  ...
  public CreateApplicationFormResponse createApplicationForm(CreateApplicationFormRequest body) {
    ...
    for (Field field : requiredFields) {
      ApplicationDefaultField applicationDefaultField =
          ApplicationDefaultField.builder().step(step).field(field).isRequired(true).build();
  ...
  public GetApplicationFormResponse getApplicationForm(String stepId) {
    ...
    // 2. 지원서 필수 응답항목 불러오기
    List<ApplicationDefaultField> applicationDefaultFields =
        applicationDefaultFieldRepository.findByStep(step);
  ...
  public CreateApplicationResponse createApplication(CreateApplicationRequest body) {
    ...
    final Applicant applicant =
        Applicant.builder().recruitment(step.getRecruitment()).club(club).build();
    ...
    // 3. 지원서 생성
    Application application = Application.builder().applicant(applicant).step(step).build();
    applicationRepository.save(application);

    // 4. 질문 응답 생성
    List<CreateApplicationRequest.QuestionAnswerDto> answers = body.answers();
    if (answers.isEmpty()) throw new IllegalArgumentException("Answers cannot be empty");
      ...
      if (question.getQuestionType() == QuestionType.MULTIPLE_CHOICE) {
        ...
        MultipleChoiceOption multipleChoiceOption =
            multipleChoiceOptionRepository
                .findById(answerDto.optionAnswerId())
  ...
  @Override
  @Transactional
  public List<GetApplicationResponse> findApplicationByApplicantId(
    ...
    if (applicants.isEmpty()) throw new IllegalArgumentException("Applicant not found");

    List<Application> applications = applicationRepository.findAllByApplicantIn(applicants);
    if (applications.isEmpty()) throw new IllegalArgumentException("application not found");
    ...
    for (Application application : applications) {
      ...
      for (Answer answer : answers) {
        ...
        // 동아리원 접근 제한 응답 확인
        if (userClubRole.getClubRole() == ClubRole.MEMBER && !question.isAccessible()) continue;

        String answerText = getAnswerText(question, answer);
        GetApplicationResponse.QuestionAnswerDto questionAnswerDto =
            GetApplicationResponse.QuestionAnswerDto.builder()
      ...
      SortUtils.sortList(
          questionAnswerDtos,
          Comparator.comparing(GetApplicationResponse.QuestionAnswerDto::questionOrder));
      ...
      else requiredFieldAnswerDto = application.getApplicant().toNameOnlyRequiredFieldDto();
  ...
  @Override
  @Transactional
  public UpdateAnswerAccessibilityResponse updateAnswerAccessibility(
