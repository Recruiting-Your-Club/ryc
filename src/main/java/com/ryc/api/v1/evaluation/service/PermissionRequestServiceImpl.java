...
public class PermissionRequestServiceImpl implements PermissionRequestService {
    ...
    @Override
    @Transactional
    public UpdatePermissionStatusResponse updatePermissionApplicationStatus(UpdatePermissionStatusRequest body) {
        //1. 평가권한 request 찾기
        PermissionApplication permissionApplication = permissionRequestRepository.findById(body.permissionApplicationId())
        ...
        if (requestStatusBeforeUpdate == body.requestStatus())
            throw new IllegalArgumentException("Requested status is the same as the current status. No update is necessary.");
        ...
        UpdateStatusInformation updateStatusInformation = UpdateStatusInformation.builder()
        ...
        if (permissionApplication.getRequestStatus() == RequestStatus.APPROVED) {
            ...
            try {
                ...
                throw new DuplicateKeyException("Duplicate entry detected: The user,recruitment combination already exists.");
            }
        }
