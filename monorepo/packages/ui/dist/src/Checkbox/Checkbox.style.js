...
export const s_size = (size = 's') => {
    return css `
        width: ${checkboxSize[size].width};
        height: ${checkboxSize[size].height};
    `;
};
export const s_text = (size = 's', disabled = false) => {
    return css `
...
const baseSVGwithColor = (isChecked = false, defaultChecked = false, disabled = false) => css `
    position: absolute;
    width: 100%;
    height: 100%;
    color: ${(defaultChecked || isChecked) && disabled && theme.colors.white};
`;
export const s_svgColor = (variant = 'outline', color = 'default', isChecked = false, defaultChecked = false, disabled = false) => {
    switch (variant) {
        case 'outline':
        case 'subtle':
            return css `
...
const baseVariant = (isChecked = false, defaultChecked = false, disabled = false) => css `
    display: flex;
    position: relative;
    align-items: center;
    ...
    `}
`;
export const s_variant = (isChecked = false, variant = 'outline', color = 'default', defaultChecked = false, disabled = false) => {
    switch (variant) {
        case 'outline':
            return css `
