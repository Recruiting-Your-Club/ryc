...
const getToggleContainerSize = (size) => {
    switch (size) {
        case 'sm':
            return css `
                width: 3.5rem;
...
export const hiddenCheckbox = css `
    ...
    height: 0;
`;
export const toggleCircle = (isChecked, size) => css `
    ${getToggleSize(size)};
    border-radius: 50%;
...
`;
export const toggleContainer = (isChecked, size) => css `
    display: flex;
    align-items: center;
    cursor: pointer;
    padding: 0 0.2rem;
